...
let database = angular.module("database.services", [
  "ngResource",
  SpinnerService
]);
...
database.factory("Database", [
  ...
  "localStorageService",
  "SchemaService",
  function(DatabaseApi, localStorageService, SchemaService) {
    var version =
    ...
    return {
      ...
      getPropertyFromDoc: function(doc, includeLinks) {
        ...
        var all = Object.keys(doc).filter(function(element, index, array) {
          ...
          if (isGraph) {
            return (
              fixedHeader.indexOf(element) == -1 &&
              (!element.startsWith("in_") && !element.startsWith("out_")) &&
              !self.isLink(type)
            );
          } else {
        ...
        fields.forEach(function(elem, index, array) {
          if (all.indexOf(elem) == -1) {
            ...
            if (isGraph) {
              bool =
                fixedHeader.indexOf(elem) == -1 &&
                (!elem.startsWith("in_") && !elem.startsWith("out_")) &&
                !self.isLink(type);
      ...
      getPropertyTableFromResults: function(results) {
        ...
        if (temp) {
          if (all.indexOf("@rid") != -1) all.splice(all.indexOf("@rid"), 1);
      ...
      getClusterSelection: () => {
        return (
          current.metadata.server.clusterSelectionStrategies || [
            "round-robin",
            "default",
            "balanced",
            "local"
...
database.factory("DatabaseApi", [
  "$http",
  "$resource",
  "$q",
  function($http, $resource, $q) {
    var urlWiki =
      "https://github.com/orientechnologies/orientdb-studio/wiki/Functions";
    ...
    resource.install = function(db, username, password) {
      ...
      $http
        .post(API + "installDatabase", db.url)
        .success(function(data) {
          deferred.resolve(data);
        })
        .error(function(data) {
    ...
    resource.createDatabase = function(
      ...
      $http
        .post(API + "database/" + name + "/" + stype + "/" + type)
        .success(function(data) {
          delete $http.defaults.headers.common["Authorization"];
          callback(data);
        })
...
database.factory("CommandApi", [
  "$http",
  "$resource",
  "Notification",
  "Spinner",
  "$q",
  function($http, $resource, Notification, Spinner, $q) {
      ...
      if (contentType == "text/csv") {
        ...
        $http
          ...
          .success(function(data) {
            ...
            var form = $(
              '<a style="display: none;" type="hidden" id="linkdownload" href="data:application/octet-stream;charset=utf-8;base64,' +
            ...
            document
              .getElementById("linkdownload")
              .setAttribute("download", namefile + ".csv");
...
database.factory("DocumentApi", [
  "$http",
  "$resource",
  "Database",
  "$q",
  function($http, $resource, Database, $q) {
    ...
    resource.uploadFileDocument = function(
      database,
      doc,
      blob,
      name,
...
database.factory("FunctionApi", [
  "$http",
  "$resource",
  "Notification",
  function($http, $resource, Notification) {
    ...
    resource.executeFunction = function(params, callback, error) {
      ...
      if (params.parameters == "") {
        var text =
          API + "function/" + params.database + "/" + params.functionName;
      } else {
        var text =
          API +
          "function/" +
...
database.factory("DatabaseAlterApi", [
  "$http",
  "$resource",
  "$q",
  function($http, $resource, $q) {
...
database.factory("ClassAlterApi", [
  "$http",
  "$resource",
  "$q",
  function($http, $resource, $q) {
...
database.factory("PropertyAlterApi", [
  "$http",
  "$resource",
  "$q",
  function($http, $resource, $q) {
